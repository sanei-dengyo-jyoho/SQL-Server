...
,
	case
		when isnull(s1.過去経験,0) <> 0
		then
			concat(
				N' ',
				N'過去',
				case
					when isnull(s1.過去経験年,0) <> 0
					then
						concat(
...
,
	case
		when (isnull(t11.講習受講1,0) + isnull(t11.講習受講2,0)) = 0
		then 0
		else 1
	end
	as 講習受講
...
,
	isnull(t2.[№],0) +
	case
		when isnull(s1.監理技術者,0) = 0
		then 0
		else 1
	end
	+
	case
		when isnull(t15.建設業経理事務士,0) = 0
		then 0
		else 1
	end
	as [資格証№]

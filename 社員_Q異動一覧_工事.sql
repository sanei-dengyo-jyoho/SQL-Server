...
,	s1.過去経験年
,	s1.過去経験月
,	case isnull(s1.過去経験,0) when 0 then '' else ' '+N'過去'+case isnull(s1.過去経験年,0) when 0 then '' else convert(varchar(3),isnull(s1.過去経験年,0))+N'年' end+case isnull(s1.過去経験月,0) when 0 then '' else convert(varchar(3),isnull(s1.過去経験月,0))+N'ヶ月' end end as 過去経験年月
...
,	case when isnull(t15.建設業経理事務士,9) > 2 then '' else dbo.SqlStrConv(t15.建設業経理事務士,4)+N'級' end as 経理士

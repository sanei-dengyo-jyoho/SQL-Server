...
,	a0.事業所レコード順序
,
	SUBSTRING(a0.事業所レコード順序,1,CHARINDEX(N'@',a0.事業所レコード順序)-1)
	as 事業所順序
,	a0.会社コード
,	a0.順序コード
,	a0.本部コード
...
from
		...
		convert(nvarchar(1000),
			CASE
				...
				THEN
					concat(
						ISNULL(s01.会社コード,''),
						CONVERT(nvarchar(5),10000+ISNULL(s01.順序コード,0)),
						N'@',
						N'本社'
					)
				ELSE
					concat(
						ISNULL(s01.会社コード,''),
						CONVERT(nvarchar(5),10000+ISNULL(s01.順序コード,0)),
						CONVERT(nvarchar(5),10000+ISNULL(s01.本部コード,0)),
						CONVERT(nvarchar(5),10000+ISNULL(s01.部コード,0)),
						CONVERT(nvarchar(5),10000+ISNULL(s01.課コード,0)),
						CONVERT(nvarchar(5),10000+ISNULL(s01.所在地コード,0)),
						N'@',
						ISNULL(s01.場所名,N'')
					)
			END
	...
	,	s01.本部名
	,	s01.部名
	,	format(e01.社員コード,'D4') as 社員コードキー
	,	e01.社員コード
	,	e01.氏名
	,	e01.カナ氏名
	,	e01.職制区分
